...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr  5 22:36:51 2020
...
#    file_dir = str(directory_filebase)
    file_ori = arm.read_netcdf(directory_filebase)
...
#   obj = arm.read_netcdf(files,variables=var)
    obj = arm_read_netcdf_for_time_resolution(files,'1s')
    diff = obj[var].diff('time', n=1)

    time = obj['time'].values

    ds = diff.to_dataset(name='co_diff')
    ds['co_diff'].values = abs(diff.values)
...
#    S = int(0.5*N)
    ...
    return index
#%%
index_cpc_ml= machine_learning(''.join(['/Users/qingn/Desktop/NQ/',datastream,'/*','20171113','*']) ,''.join(['/Users/qingn/Desktop/NQ/',datastream,'/mar*.nc'])) 

#/Users/qingn/Desktop/NQ
